...
void UWBAbilitySystemComponent::OnAbilityInputPressed(const FGameplayTag& InInputTag)
{
	...
	for (const FGameplayAbilitySpec& AbilitySpec : GetActivatableAbilities())
	{
		...
		if (InInputTag.MatchesTag(WBGameplayTags::InputTag_Toggleable) && AbilitySpec.IsActive())
		{
...
void UWBAbilitySystemComponent::GrantHeroWeaponAbilities(const TArray<FWBHeroAbilitySet>& InDefaultWeaponAbilities, const TArray<FWBHeroSpecialAbilitySet>& InSpecialWeaponAbilities, int32 ApplyLevel, TArray<FGameplayAbilitySpecHandle>& OutGrantedAbilitySpecHandles)
{
	if (InDefaultWeaponAbilities.IsEmpty()) return;

	for (const FWBHeroAbilitySet& AbilitySet : InDefaultWeaponAbilities)
	{
	...
	for (const FWBHeroSpecialAbilitySet& AbilitySet : InSpecialWeaponAbilities)
	{
		if (!AbilitySet.IsValid()) continue;
